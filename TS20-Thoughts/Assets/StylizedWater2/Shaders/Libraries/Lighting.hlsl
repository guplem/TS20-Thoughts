...
float3 ApplyLighting(inout SurfaceData surfaceData, InputData inputData, TranslucencyData translucencyData, SurfaceNormalData normalData, float shadowMask, float vFace)
{
	#if _TRANSLUCENCY
	...
	MixRealtimeAndBakedGI(mainLight, normalWS, inputData.bakedGI, shadowMask.xxxx);
	...
	half3 diffuseColor = GlobalIllumination(brdfData, inputData.bakedGI, shadowMask, inputData.normalWS, inputData.viewDirectionWS);
	...
	*/
	...
	half specularPower = (_PointSpotLightReflectionExp * 0.03125); //= 1.0/32
	...
	for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
	{
		...
		TranslucencyData lightTranslucencyData = PopulateTranslucencyData(translucencyData.subsurfaceColor, light.direction, attenuatedLightColor, inputData.viewDirectionWS, normalData.geometryNormalWS, normalData.pixelNormalWS, translucencyData.mask , translucencyData.strength * 16 * (light.distanceAttenuation));
		ApplyTranslucency(lightTranslucencyData, surfaceData.emission.rgb);
		#endif
	...
	color = (surfaceData.albedo.rgb * diffuseColor) + surfaceData.emission.rgb + surfaceData.specular;
#endif
