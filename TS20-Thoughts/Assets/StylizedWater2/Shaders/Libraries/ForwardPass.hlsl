...
#endif
{
	...
	albedo.rgb = lerp(albedo.rgb, reflections, _ReflectionStrength * reflectionFresnel * vFace);
	//return float4(albedo.rgb, 1);
#endif
	...
	#endif
	albedo = lerp(albedo + caustics, albedo, causticsMask);
#endif
	...
	SampleDiffuseProjectors(albedo.rgb, wPos, ScreenPos);
	...
	float4 finalColor = float4(ApplyLighting(surfaceData, inputData, translucencyData, normalData, _ShadowStrength, vFace), alpha);
